...
import { innerChromeProfiles } from './profile-runtime.js';
...
/** Are all required Playwright patches present? This is the load-bearing check. */
...
export function agentBrowserState() {
  const result = runAgentBrowser(['--version'], {
    encoding: 'utf8',
    stdio: ['ignore', 'pipe', 'pipe'],
  });
  const v = result.status === 0 ? (result.stdout || '').trim().split(/\s+/).pop() || '' : '';
...
export function warnIfDegraded({ prefix = 'web-plane' } = {}) {
  ...
  if (patch.ok && !clone.ok) {
    if (!clone.adhoc) {
      ...
      // both fact and direction. Version lag is advisory, as it is in doctor;
      // it does not prevent DYLD injection or hidden-window operation.
      console.error(
...
export function doctor() {
  ...
  const patch = patchState();
  const patchedFiles = PATCH_MARKERS.map(({ file }) => file.split('/').pop()).join(' + ');
  rows.push(
    patch.ok
      ? ['✓', 'playwright patch', `applied (${patchedFiles})`, null]
      : [
          '✗',
          'playwright patch',
  ...
  if (!ab.installed) {
    rows.push(['✗', 'agent-browser', 'package dependency missing or broken', 'reinstall web-plane']);
    bad++;
  } else {
    rows.push(
      ab.ok
  ...
  if (existsSync(paths.profilesDir)) {
    for (const entry of readdirSync(paths.profilesDir, { withFileTypes: true })) {
      ...
      const profiles = innerChromeProfiles(join(paths.profilesDir, entry.name));
      if (profiles.length > 1) splits.push(`${entry.name} (${profiles.join(', ')})`);
    }
  }
  rows.push([
    splits.length ? '⚠' : '✓',
    'inner profiles',
    splits.length
      ? `Default selected; legacy splits: ${splits.join('; ')}`
      : 'Default selected; no splits detected',
    null,
  ]);
