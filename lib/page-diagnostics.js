...
async function evaluateContexts(connection, contexts, expression, { includeCommandLineAPI = false } = {}) {
  const results = [];
...
export async function evalAllFrames(port, targetId, expression, options = {}) {
  return pageContext(port, targetId, async ({ connection, contexts }) =>
    evaluateContexts(connection, contexts, expression, options)
  );
}
