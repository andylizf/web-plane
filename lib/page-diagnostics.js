...
export function parseAgentBrowserValueLength(stdout) {
  try {
    const response = JSON.parse(stdout);
    if (response?.success !== true) return null;
    const value = response?.data?.value ??
      (typeof response?.data === 'string' ? response.data : null);
    return typeof value === 'string' || typeof value === 'number' ? String(value).length : null;
  } catch {
    return null;
  }
}
...
export function fieldChangeSummary(operation, before, after) {
  const action = operation === 'append'
    ? 'appended field content'
    : operation === 'clear'
      ? 'cleared field content'
      : 'replaced field content';
  const from = Number.isInteger(before) ? before : 'unknown';
  const to = Number.isInteger(after) ? after : 'unknown';
  return `${action} (length ${from} -> ${to}; values hidden)`;
}
