...
export function recallLane(lane) {
  ...
  if (current) {
    if (current.version === STATE_VERSION && !Object.hasOwn(current, 'keep')) return current;
    const { keep: _legacyKeep, ...rest } = current;
  ...
  const { keep: _legacyKeep, ...legacyState } = legacy;
