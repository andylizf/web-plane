...
export function parseAttachOptions(args) {
  const flag = takeFlag(args, '--no-webauthn');
  if (flag.matches.length > 1 || flag.matches.some(value => value !== true)) {
    throw new Error('--no-webauthn is a flag and may be supplied only once');
  }
  const shown = takeFlag(flag.rest, '--while-shown');
  if (shown.matches.length > 1 || shown.matches.some(value => value !== true)) {
    throw new Error('--while-shown is a flag and may be supplied only once');
  }
  return { args: shown.rest, webauthnDisabled: flag.matches.length === 1, whileShown: shown.matches.length === 1 };
}
...
function navigationWait(args, defaultWaitFor) {
  ...
  const condition = waitFor.value ?? defaultWaitFor;
...
export function parseSemanticFind(args) {
  ...
  if (rest.length === 0) {
    throw new Error(
      "lane find role requires an action: check, click, fill, hover, or text (only 'text' reads)"
    );
  }
  const action = rest[0];
...
export function translateLaneCommand(inputArgs, { defaultWaitFor = 'networkidle' } = {}) {
  ...
  if (command === 'keep' || command === 'unkeep') {
    throw new Error(
      `lane ${command} was removed; close the lane when its task ends, or let the hard idle ` +
      'timeout reclaim an abandoned lane'
    );
  }
  if (NAVIGATION_COMMANDS.has(command)) {
    const parsed = navigationWait(args.slice(1), defaultWaitFor);
...
Lane input:
  ...
  find role <role> <check|click|fill|hover|text> --name <name>
                               Resolve by accessible role/name instead of stale refs;
                               the action is required and only 'text' reads
...
Inspection and diagnosis:
  ...
  screenshot [path]            Read the page visually: canvas content, and any
                               action that reported success without advancing
...
Lifecycle:
  close                        Close this tab and lane driver; sibling lanes survive

Every lane is force-closed after a 24-hour hard idle timeout, including visible lanes.
Maximum Memory Saver is enabled separately and may reload an inactive tab before that deadline.
Test-only timing overrides: WEB_PLANE_LANE_TTL_MS and WEB_PLANE_REAP_INTERVAL_MS.

While the session window is shown the user owns it: page-changing commands are
refused with WINDOW_SHOWN, and snapshot/get/is/console/errors/title/url read the
lane without bringing its tab forward. Put --while-shown before the command
(lane <lane> --while-shown click @e1) only after the user agrees.
