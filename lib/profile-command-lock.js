  ...
  while (true) {
    ...
    if (Date.now() >= deadline) {
      const error = new Error(
        `Chrome profile '${session}' command critical section is still occupied after waiting ` +
        `${timeoutMs / 1000}s; a long-running lane command holds it until that command finishes`
      );
