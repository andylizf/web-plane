...
function laneIsActive(name) {
  ...
  return activeTabLabel(res.stdout) === name;
}

/**
 * The label of the tab that `tab list` marks as active, or null when the
 * listing does not clearly say so.
 *
 * Split out from the spawn above so it can be tested against real listings.
 * Every branch that returns null is a decision to re-pin: an unparseable
 * listing must fall through to pinning, because driving the wrong tab is worse
 * than losing the ref table.
 */
export function activeTabLabel(stdout) {
  if (!stdout) return null;
  // eslint-disable-next-line no-control-regex
  const plain = stdout.replace(/\[[0-9;]*m/g, '');
  const active = plain.split('\n').find((l) => l.trimStart().startsWith('→'));
  if (!active) return null;
  return active.match(/\[t\d+\]\s+(\S+)/)?.[1] ?? null;
}
