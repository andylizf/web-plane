...
import { normalizeSpawnTimeout, runAgentBrowser } from './agent-browser.js';
...
function runAttachDriver(session, lane, args) {
...
async function recoverLaneBinding(name, known) {
  const { port } = await cdp(known.session);
  let targets;
  try {
  ...
  const drive = (args) => runAttachDriver(known.session, name, args);
...
export async function attach(session, urlOrArgs, lane = null) {
  ...
  const { port, reused, sessionRestoreAttempted } = await cdp(session);

  const drive = (args) => runAttachDriver(session, lane, args);
  ...
  try {
    ...
    if (claimed.status !== 0) {
      ...
      // replaces.
      if (!reused && !sessionRestoreAttempted) drive(['tab', 'close', 't1']);
    }
...
export async function lane(name, args) {
  ...
  if (translated.operation === 'keep-lane' || translated.operation === 'unkeep-lane') {
    let releaseKeepLock;
    try {
      releaseKeepLock = await acquireProfileCommandLock(known.session);
      const keep = translated.operation === 'keep-lane';
      setLaneKeep(name, keep);
      console.log(
        keep
          ? `Lane '${name}' is kept from automatic reclamation.`
          : `Lane '${name}' automatic reclamation enabled.`
      );
      process.exitCode = 0;
      return;
    } catch (error) {
      console.error(`web-plane: could not update lane '${name}': ${error.message}`);
      process.exitCode = 1;
      return;
    } finally {
      if (releaseKeepLock) releaseKeepLock();
    }
  }
