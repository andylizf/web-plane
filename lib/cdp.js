...
import { isCoveredClickFailure, translateLaneCommand } from './lane-commands.js';
...
async function runLaneCommand(name, args, known, port, wait = null) {
  ...
  const result = runAgentBrowserLaneCommand(name, args);
...
export async function lane(name, args) {
  ...
  let status;
  try {
    status = await runLaneCommand(name, args, known, port, translated.wait);
  } finally {
    releaseLock();
  }
  if (status === 0 && translated.operation === 'close-lane') {
    try { unlinkSync(laneFile(name)); } catch {}
  }
  process.exit(status);
}
