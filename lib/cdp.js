...
import { holdLaneFocus } from './lane-focus.js';
import { findChrome } from './procs.js';
import { sessionShown } from './window.js';
...
export async function attach(session, urlOrArgs, lane = null) {
  ...
  // A new lane tab would open over whatever the user is looking at.
  if (reused && !whileShown && await isShown(session)) {
    printShownRefusal(session, 'attach', "'attach --while-shown'");
    releaseLock();
    process.exit(4);
  }
...
async function activateLaneTarget(name, port, { activate = true } = {}) {
  ...
  if (activate) await activateCdpTarget(port, targetId);
...
// Commands that only read the pinned tab. While the user holds the window
// these still run, but without activating the lane's tab over theirs.
const READ_ONLY_WHILE_SHOWN = new Set(['snapshot', 'get', 'is', 'console', 'errors', 'title', 'url']);

async function isShown(session) {
  try {
    return await sessionShown(findChrome(session));
  } catch {
    return false;
  }
}

function printShownRefusal(session, command, override) {
  printUIResult({
    ok: false,
    error: {
      code: 'WINDOW_SHOWN',
      message: `session '${session}' is shown, so the user may be working in it; '${command}' was not sent`,
      command,
      commandExecuted: false,
      next: `Wait with 'web-plane -s=${session} wait-hidden', or ask the user. ` +
        `Only after they agree, rerun with ${override}.`,
    },
  });
}
 ...
 * id. The lane record follows it (the monitor updates it), but the driver is
 * still pinned to the old id; re-pin it before giving the command up.
 */
async function rebindDriverToReplacement(name, known, port) {
  const record = recallLane(name) ?? known;
  let targets;
  try {
    targets = await listRestoredPageTargets(port);
  } catch {
  ...
  let target = targets.find((candidate) => candidate.id === record.targetId) ?? null;
  if (!target) {
    const resolution = resolveRestoredTarget(
      record,
      targets.filter((candidate) => candidate.id !== known.targetId)
    );
    if (resolution.status !== 'matched') return false;
    target = resolution.target;
    updateLaneTarget(name, { targetId: target.id, url: target.url, title: target.title });
  }
  // A discarded placeholder has no renderer and answers no page command. In
  // this browser activating it does not make Chrome reload it (measured: the
  // document never appears), so open the lane's URL in a fresh tab, let it
  // load, and drop the placeholder; the monitor follows the new target.
  try {
    await activateCdpTarget(port, target.id);
    let ready = await targetRendererReady(port, target.id, 3_000);
    if (!ready) {
      const fresh = await openTabForLane(port, target.url || record.url);
      await waitForTargetRenderer(port, fresh.id, 15_000);
      await closePinnedLaneTarget(port, target.id);
      updateLaneTarget(name, { targetId: fresh.id, url: fresh.url, title: fresh.title });
      appendSessionEvent(known.session, {
        type: 'lane-tab-recreated',
        lane: name,
        from: target.id,
        targetId: fresh.id,
        url: fresh.url,
    ...
    appendSessionEvent(known.session, {
      type: 'lane-driver-rebind-failed',
      lane: name,
      targetId: target.id,
  ...
  const pinned = runAttachDriver(known.session, name, ['--pin-tab', 'tab', target.id], port);
  if (pinned.status !== 0) return false;
  appendSessionEvent(known.session, {
    type: 'lane-driver-rebound',
    lane: name,
    from: known.targetId,
    targetId: target.id,
  });
  console.error(
    "web-plane: Chrome had discarded the lane's idle tab; it was reopened from its URL and the driver re-pinned."
  );
  return true;
}

/** Open the lane's URL in a new tab of the same window and return its target. */
async function openTabForLane(port, url) {
  if (!url) throw new Error('the lane record has no URL to reopen');
  const connection = await CdpConnection.connect(port, 2_000);
  try {
    const { targetId } = await connection.send('Target.createTarget', { url }, null, 5_000);
    const { targetInfo } = await connection.send('Target.getTargetInfo', { targetId }, null, 2_000);
    return { id: targetId, url: targetInfo?.url || url, title: targetInfo?.title ?? '' };
  } finally {
    connection.close();
  }
}

async function targetRendererReady(port, targetId, timeoutMs) {
  try {
    await waitForTargetRenderer(port, targetId, timeoutMs);
    return true;
  } catch {
    return false;
  }
...
/** Resolve once the target's document answers, i.e. its renderer exists again. */
async function waitForTargetRenderer(port, targetId, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      lastError = `readyState ${result?.value}`;
    } catch (error) {
  ...
  throw new Error(`the reloaded tab did not become ready within ${timeoutMs}ms (${lastError})`);
}

async function runLaneCommand(name, args, known, port, operation, wait = null, { activate = true } = {}) {
  ...
  try {
    targetId = await activateLaneTarget(name, port, { activate });
    targetActive = true;
  } catch (error) {
    if (await rebindDriverToReplacement(name, known, port)) {
      try {
        targetId = await activateLaneTarget(name, port, { activate });
        targetActive = true;
      } catch {}
...
export async function lane(name, args) {
  ...
  const whileShown = args[0] === '--while-shown';
  if (whileShown) args = args.slice(1);
  let translated;
  try {
  ...
  // Swapping the visible tab or typing into a page the user is looking at
  // takes the browser out of their hands, whichever agent owns the lane.
  const shown = !whileShown && await isShown(known.session);
  if (shown && !READ_ONLY_WHILE_SHOWN.has(args[0])) {
    printShownRefusal(known.session, args[0], `'web-plane lane ${name} --while-shown ${args[0]} ...'`);
    releaseLock();
    process.exit(4);
  }

  let status;
  const focus = await holdLaneFocus(port, known.targetId);
  if (!focus.held && focus.reason) {
    appendSessionEvent(known.session, { type: 'focus-hold-unavailable', lane: name, message: focus.reason });
  }
  try {
    status = await runLaneCommand(name, args, known, port, translated.operation, translated.wait, { activate: !shown });
