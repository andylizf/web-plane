...
import { patchState, agentBrowserState, cloneRefresh, isNewer } from './health.js';
...
export async function install() {
  ...
  // The same call `doctor` prints its clone-version row from, so the two cannot
  // send the user in a circle — doctor warning "≠ system, fix: web-plane install"
  // while install answered "up to date" and did nothing. It also says which way
  // the versions differ; only a clone behind the system is worth re-cloning.
  ...
  if (refresh.needed) {
    ...
    console.log(`==> Chrome clone up to date: ${refresh.why}`);
  }
