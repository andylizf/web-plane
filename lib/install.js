...
import { enableMaximumMemorySaverForAllProfiles } from './profile-runtime.js';
...
export async function install() {
  ...
  const memorySaverProfiles = enableMaximumMemorySaverForAllProfiles();
  const memorySaverChanged = memorySaverProfiles.filter((profile) => profile.changed).length;
  note(
    `==> Chrome Maximum Memory Saver enabled for ${memorySaverProfiles.length} managed ` +
      `profile${memorySaverProfiles.length === 1 ? '' : 's'}` +
      (memorySaverChanged ? ` (${memorySaverChanged} updated)` : '')
  );
