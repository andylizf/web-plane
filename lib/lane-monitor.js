...
import { LANE_FOCUS_IDLE_MS, LANE_TTL_MS, paths, REAP_INTERVAL_MS } from './config.js';
import { stopAgentBrowserDaemon } from './agent-browser.js';
import { CdpConnection } from './cdp-client.js';
import { laneFocusDecision, laneReapDecision, nextLaneSweepDelay } from './lane-lifecycle.js';
import { listRestoredPageTargets, resolveDiscardReplacement } from './lane-recovery.js';
import { appendLaneEvent, laneEventsPath } from './lane-events.js';
import { forgetLane, laneStatePaths, recallLane, updateLaneTarget } from './lane-state.js';
...
/** Whether the detached monitor recorded for this lane is still running. */
export function laneMonitorAlive(lane) {
  try {
    const state = JSON.parse(readFileSync(laneMonitorPaths(lane).pid, 'utf8'));
...
export async function startLaneMonitor({ lane, session, port, targetId }) {
  ...
  const config = { lane, session, port, targetId, token,
    webauthnDisabled: recallLane(lane)?.webauthnDisabled === true };
...
async function runWorker(config) {
  const { lane, session, port, token } = config;
  // Reassigned when Memory Saver replaces the tab with a placeholder target.
  ...
  // True from a discard until the replacement tab has a renderer again: a
  // placeholder answers no page command, so nothing is sent to it meanwhile.
  let placeholder = false;
  let instrumenting = null;
  const recentPages = [];
  ...
  const enableSession = async (sessionId, timeoutMs = 5_000) => {
    await connection.send('Runtime.enable', {}, sessionId, timeoutMs);
    await connection.send('Network.enable', {}, sessionId, timeoutMs);
    await connection.send('Page.enable', {}, sessionId, timeoutMs);
  };

  // Once Chrome has reloaded the replacement (it does so on activation),
  // attach to it and enable the domains as at start-up. Before that every
  // page command hangs, so a failed attempt is silent and retried later.
  const instrumentReplacement = () => {
    if (!placeholder || instrumenting) return instrumenting ?? Promise.resolve();
    instrumenting = (async () => {
      let sessionId = null;
      try {
        sessionId = await connection.attachTarget(targetId);
        ...
        await enableSession(sessionId, 2_000);
        const { frameTree } = await connection.send('Page.getFrameTree', {}, sessionId, 2_000);
        mainSession = sessionId;
        mainFrameId = frameTree?.frame?.id ?? null;
        placeholder = false;
        record({ type: 'replacement-instrumented', targetId });
      } catch {
        if (sessionId) {
          try { await connection.send('Target.detachFromTarget', { sessionId }, null, 1_000); } catch {}
        }
      } finally {
        instrumenting = null;
      }
  ...
  // Memory Saver discards an idle tab by replacing its WebContents: Chrome
  // creates a placeholder with the same URL and title under a new target id
  // just before destroying the old one. Follow it, or the reaper and the next
  // command would both be looking for a tab that no longer exists.
  const adoptReplacement = async (oldId) => {
    await new Promise((resolveWait) => setTimeout(resolveWait, 300));
    if (stopping) return;
    const state = recallLane(lane);
    const recent = recentPages.filter((page) => Date.now() - page.at < 10_000);
    let resolution = resolveDiscardReplacement(state, recent, oldId);
    if (resolution.status !== 'matched') {
      let live = [];
      try { live = await listRestoredPageTargets(port); } catch {}
      resolution = resolveDiscardReplacement(state, live, oldId);
    }
    if (resolution.status !== 'matched') {
      targetDestroyed = true;
      record({ type: 'target-lost', targetId: oldId, replacement: resolution.reason });
      return;
    }
    const next = resolution.target;
    targetId = next.id;
    mainSession = null;
    mainFrameId = null;
    focusHeld = false;
    placeholder = true;
    updateLaneTarget(lane, { targetId: next.id, url: next.url, title: next.title });
    record({ type: 'target-replaced', from: oldId, to: next.id, url: next.url });
  };

  connection.onEvent((message) => {
    const params = message.params ?? {};
    if (message.method === 'Target.targetCreated' && params.targetInfo?.type === 'page') {
      recentPages.push({
      ...
      if (recentPages.length > 8) recentPages.shift();
      return;
    }
    ...
    if (message.method === 'Target.targetDestroyed' && params.targetId === targetId) {
      adoptReplacement(params.targetId).catch((error) => {
        targetDestroyed = true;
        record({ type: 'monitor-error', operation: 'adopt-replacement', message: error.message });
      });
      return;
    }
    if (message.method === 'Target.targetInfoChanged' && params.targetInfo?.type === 'page') {
      const seen = recentPages.find((page) => page.id === params.targetInfo.targetId);
      if (seen) Object.assign(seen, { url: params.targetInfo.url ?? '', title: params.targetInfo.title ?? '' });
    }
    if (message.method === 'Target.targetInfoChanged' && params.targetInfo?.targetId === targetId) {
      ...
      if (placeholder) instrumentReplacement().then(() => { if (!placeholder) scheduleFocus?.(); });
      return;
    }
    if (
  ...
  if (config.webauthnDisabled) {
    // Keep the virtual environment empty: adding an authenticator could let a
    // site register a credential that disappears when this monitor disconnects.
    await connection.send('WebAuthn.enable', { enableUI: false }, mainSession);
  }
  await connection.send(
    'Target.setAutoAttach',
    { autoAttach: true, waitForDebuggerOnStart: false, flatten: true },
  ...
  // Focus emulation makes Chrome count the tab as captured, which keeps it
  // visible and rendering at full rate. Hold it while the lane is active so
  // pages behave as focused for commands, and release it once the lane idles
  // so a background tab goes hidden and Chrome's own throttling applies. The
  // handle belongs to this session, so a dead monitor cannot leave it held.
  // One write to the lane record raises several watcher events; serialize so
  // they cannot race past the held/released check and log the same hold twice.
  let focusOp = Promise.resolve();
  const setFocus = (enabled, reason) => {
    focusOp = focusOp.then(async () => {
      if (focusHeld === enabled || !mainSession) return;
      await connection.send('Emulation.setFocusEmulationEnabled', { enabled }, mainSession);
      focusHeld = enabled;
      record({ type: enabled ? 'focus-held' : 'focus-released', targetId, reason });
    });
    return focusOp;
  };
  scheduleFocus = async () => {
    ...
    const decision = laneFocusDecision({ state: recallLane(lane), idleMs: LANE_FOCUS_IDLE_MS });
    // A command against a discarded tab reloads it first; that is the moment
    // the replacement can be instrumented, and focus follows only then.
    if (placeholder && decision.hold) await instrumentReplacement();
    try {
      await setFocus(decision.hold, decision.hold ? 'lane-active' : 'lane-idle');
    } catch (error) {
      record({ type: 'monitor-error', operation: 'focus-emulation', message: error.message });
    }
    if (decision.hold && !stopping) focusTimer = setTimeout(scheduleFocus, decision.releaseInMs);
  };
  // The CLI rewrites the lane record before every command, so a change to that
  // file is the signal that the lane is active again.
  const lanePaths = laneStatePaths(lane);
  try {
    stateWatcher = watch(lanePaths.dir, (_event, filename) => {
      if (filename === basename(lanePaths.state)) scheduleFocus();
    });
    stateWatcher.unref?.();
  } catch (error) {
    record({ type: 'monitor-error', operation: 'watch-lane-state', message: error.message });
  }
  await scheduleFocus();
  ...
  const closeReapTarget = async () => {
    if (targetDestroyed) return { closed: true, reason: 'already-closed' };
    try {
    ...
    try {
      ...
      if (targetDestroyed) return { closed: true, reason: 'hard-timeout' };
      return { closed: false, reason: 'close-failed', error: error.message };
    }

    const deadline = Date.now() + 2_000;
    while (Date.now() < deadline) {
      // Closing the last page can end Chrome's CDP connection. Its destruction
      // event already confirms the close even when a later query cannot reply.
      if (targetDestroyed) return { closed: true, reason: 'hard-timeout' };
      try {
        const { targetInfos } = await connection.send('Target.getTargets', {}, null, 2_000);
        if (!targetInfos.some((target) => target.targetId === targetId)) {
