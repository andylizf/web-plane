...
async function runWorker(config) {
  ...
  let resolveClosed;
  // Why the monitor is unwinding decides whether the lane's agent-browser
  // daemon goes with it. A TTL reap stops the daemon itself; a SIGTERM may just
  // be startLaneMonitor replacing this worker, and the lane keeps running. Only
  // a dropped browser connection means the daemon can never be useful again.
  let exitReason = null;
  const closed = new Promise((resolveClosedPromise) => (resolveClosed = resolveClosedPromise));
  connection.onClose(() => {
    if (!stopping) {
      record({ type: 'browser-disconnected', targetId });
      exitReason = 'browser-disconnected';
    }
    stopping = true;
    if (reapTimer) {
  ...
  await closed;
  // Chrome is gone, so nothing can reach this lane's daemon again. Without this
  // it survives as a launchd-reparented orphan holding an open CDP listener --
  // one per lane per browser exit, so a long-lived install accumulates them by
  // the hundred, each invisible to the lane bookkeeping that should own it.
  if (exitReason === 'browser-disconnected') {
    const daemon = await stopAgentBrowserDaemon(lane);
    record({
      type: 'daemon-reaped-on-disconnect',
