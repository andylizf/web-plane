...
import {
  inspectLaneLifecycle,
  installLifecycleTracker,
  installLifecycleTrackerAcrossFrames,
  laneReapDecision,
} from './lane-lifecycle.js';
...
import { hiddenFlagFor } from './window.js';
...
export async function startLaneMonitor({ lane, session, port, targetId }) {
  ...
  let hiddenFlag = null;
  try {
    hiddenFlag = hiddenFlagFor(findChrome(session));
  } catch {}
  const config = { lane, session, port, targetId, token, hiddenFlag };
...
async function runWorker(config) {
  const { lane, session, port, targetId, token, hiddenFlag } = config;
  const statePaths = laneMonitorPaths(lane);
  ...
  const enableSession = async (sessionId) => {
    ...
    await installLifecycleTracker(connection, sessionId);
  };
  ...
  connection.onEvent((message) => {
    ...
    if (
      ...
      reapAttempt.beforeUnload = true;
      reapAttempt.dialogHandling = connection.send(
        'Page.handleJavaScriptDialog',
        { accept: false },
        mainSession,
        2_000
      ).catch((error) => {
        reapAttempt.dialogError = error.message;
      });
      return;
    }
    if (
      ...
      reapAttempt.dialogResult = params.result;
      reapAttempt.resolveDialogClosed();
      return;
    }
    ...
    if (message.method === 'Page.downloadWillBegin' || message.method === 'Browser.downloadWillBegin') {
      if (params.guid) downloads.add(params.guid);
    ...
    if (message.method === 'Page.downloadProgress' || message.method === 'Browser.downloadProgress') {
      if (params.guid && ['completed', 'canceled'].includes(params.state)) downloads.delete(params.guid);
      record({
  ...
  const trackerFrames = await installLifecycleTrackerAcrossFrames(port, targetId);
  if (trackerFrames.some((frame) => frame.error)) {
    record({
      type: 'monitor-error',
      operation: 'install-lifecycle-tracker',
      failedFrameCount: trackerFrames.filter((frame) => frame.error).length,
    });
  }
  ...
  // Deliberately separate from the safety inspection below. This function
  // performs the destructive step only after the caller has received an
  // explicit eligible decision while holding the profile command lock.
  const closeReapTarget = async () => {
    let resolveDialogClosed;
    const dialogClosed = new Promise((resolveDialog) => (resolveDialogClosed = resolveDialog));
    reapAttempt = {
      beforeUnload: false,
      dialogHandling: null,
      dialogError: null,
    ...
    let closeError = null;
    try {
      await connection.send('Page.close', {}, mainSession, 5_000);
    } catch (error) {
      closeError = error.message;
    }
    if (reapAttempt.dialogHandling) await reapAttempt.dialogHandling;
    if (reapAttempt.beforeUnload) {
      await Promise.race([
        dialogClosed,
    ...
    try {
      ...
      targetPresent = targetInfos.some((target) => target.targetId === targetId);
    } catch (error) {
      reapAttempt = null;
      return { closed: false, reason: 'close-not-confirmed', error: error.message };
    }
    if (reapAttempt.beforeUnload && targetPresent) {
    ...
    const attempt = reapAttempt;
    reapAttempt = null;
    if (attempt.beforeUnload) {
      if (!targetPresent) {
        return { closed: true, reason: 'beforeunload-race-closed' };
      }
      return {
        closed: false,
        reason: 'beforeunload-blocked',
        error: attempt.dialogError ?? closeError,
        targetPresent,
        dialogResult: attempt.dialogResult,
      };
    }
    if (targetPresent) return { closed: false, reason: 'close-not-confirmed', error: closeError };
    return { closed: true };
  };
  ...
  const sweep = async () => {
    ...
    try {
      ...
      let frames;
      try {
        frames = await inspectLaneLifecycle(port, targetId);
      } catch (error) {
        reapRecord('lane-reap-skipped', {
          ageMs: Number.isFinite(preliminaryCommandMs)
            ? Math.max(0, Date.now() - preliminaryCommandMs)
      ...
      // Re-read after the slower page inspection so a command or keep request
      // arriving during that inspection wins before any close is attempted.
      const current = recallLane(lane);
      const reapDecision = laneReapDecision({
        state: current,
        ttlMs: LANE_TTL_MS,
        hidden: hiddenFlag ? existsSync(hiddenFlag) : null,
        targetId,
        frames,
      ...
      if (!closeResult.closed) {
        reapRecord('lane-reap-skipped', {
          ...
          ...(Object.hasOwn(closeResult, 'targetPresent')
            ...
            : {}),
          ...(Object.hasOwn(closeResult, 'dialogResult')
            ? { dialogResult: closeResult.dialogResult }
            : {}),
      ...
      reapRecord('lane-reaped', {
        ageMs: reapDecision.ageMs,
        reason: closeResult.reason ?? 'eligible',
      });
      stopping = true;
      connection.close();
      resolveClosed();
