...
import { ensureInjectable, pruneCodeSignClones } from './sign.js';
...
export function runCommand(command, globalArgs, commandArgs) {
  ...
  if (command === 'open') {
    // Every launch leaks a Gatekeeper clone of the bundle; clear the old ones
    // before adding another. Cheap, and nothing downstream depends on it.
    pruneCodeSignClones();
    if (ensureInjectable()) {
