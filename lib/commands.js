...
import { paths, PLAYWRIGHT_LAUNCH_TIMEOUT_MS } from './config.js';
import {
  appendSessionEvent,
...
import { normalizeSpawnTimeout } from './agent-browser.js';
...
export function runCommand(command, globalArgs, commandArgs) {
  ...
  let result = spawnSync(finalArgs[0], finalArgs.slice(1), {
    stdio: 'inherit',
    env: launchEnv,
    ...(command === 'open'
      ? { timeout: PLAYWRIGHT_LAUNCH_TIMEOUT_MS, killSignal: 'SIGTERM' }
      : {}),
  });
  if (command === 'open') {
    result = normalizeSpawnTimeout(result, {
      phase: 'Playwright open',
      timeoutMs: PLAYWRIGHT_LAUNCH_TIMEOUT_MS,
    });
    if (result.timedOut && result.stderr) process.stderr.write(result.stderr);
  }
 ...
 *
 * Omitting `url` is deliberate: Chrome can restore its previous tabs only when
 * the launcher does not replace startup with a synthetic destination. A fresh
 * profile still receives Chrome's ordinary New Tab page.
 */
export function openHidden(session, url = null) {
  const pw = getPw();
  ...
  const finalArgs = [pw, ...args, 'open', ...([url].filter(Boolean))];
  let result = spawnSync(finalArgs[0], finalArgs.slice(1), {
  ...
  result = normalizeSpawnTimeout(result, {
    phase: `Playwright hidden launch for session '${session}'`,
    timeoutMs: PLAYWRIGHT_LAUNCH_TIMEOUT_MS,
  });
  if (result.timedOut) {
    appendSessionEvent(session, {
      type: 'launch-timeout',
      phase: 'playwright-hidden-launch',
      timeoutMs: PLAYWRIGHT_LAUNCH_TIMEOUT_MS,
    });
  }
