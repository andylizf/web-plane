...
// Chromium clones its own app bundle into the per-user temporary area at
// startup (the MacAppCodeSignClone feature) so that the updater can replace the
// bundle on disk without invalidating the signature of the running process. The
// clone is meant to be dropped on exit; it frequently isn't, so they accumulate
// — the same leak filed against other Chromium apps, e.g. capybara#2795 and
// codex#25667.
//
...
//
// Sweeping rather than preventing is deliberate. `--disable-features=
// MacAppCodeSignClone` stops the clones at the source, and can be merged into
// the flag Playwright already sets by way of our browserType patch. It is not
// used, for two reasons:
//
//   - The clone directory is keyed on the bundle id, so every Chrome sharing
//     `com.google.Chrome` leaks into the same place: our clone, a driver's
//     downloaded Chrome, the user's own. Disabling the feature only covers the
//     browsers we launch, while sweeping the directory clears all of them.
//   - Our clone ships Chrome's updater, and the resting state here is a
//     long-lived background browser — precisely the case the feature exists to
//     protect. Chrome for Testing can default it off because it carries no
//     updater; we cannot.
