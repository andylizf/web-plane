...
/** Persist Chrome's most aggressive background-tab deactivation policy. */
export function enableMaximumMemorySaver(session) {
  const profileDir = managedProfileDir(session);
  const localStatePath = join(profileDir, 'Local State');
  mkdirSync(profileDir, { recursive: true, mode: 0o700 });

  // Chromium registers Memory Saver as browser-wide Local State, not as a
  // profile Preference. State 2 enables it and aggressiveness 2 is Maximum.
  const localStateExisted = existsSync(localStatePath);
  const localStateOriginal = localStateExisted
    ? readFileSync(localStatePath, 'utf8')
    : '{}';
  const localState = JSON.parse(localStateOriginal);
  localState.performance_tuning = {
    ...(localState.performance_tuning ?? {}),
    high_efficiency_mode: {
      ...(localState.performance_tuning?.high_efficiency_mode ?? {}),
      state: 2,
      aggressiveness: 2,
    },
  };
  const localStateNext = JSON.stringify(localState);

  let localStateBackup = null;
  if (!localStateExisted || localStateNext !== localStateOriginal) {
    localStateBackup = localStateExisted
      ? backupFile(localStatePath, 'local-state', encodeURIComponent(session))
      : null;
    const mode = localStateExisted ? statSync(localStatePath).mode & 0o777 : 0o600;
    writeAtomic(localStatePath, localStateNext, mode);
  }

  return {
    changed: !localStateExisted || localStateNext !== localStateOriginal,
    localStateBackup,
    localState: localStatePath,
  };
}

export function enableMaximumMemorySaverForAllProfiles() {
  ...
  return readdirSync(paths.profilesDir, { withFileTypes: true })
    ...
    .map((entry) => ({
      session: entry.name,
      ...enableMaximumMemorySaver(entry.name),
    }));
}
...
export function prepareManagedProfile(session) {
  ...
  const memorySaver = enableMaximumMemorySaver(session);
  ...
  return {
    changed:
      ...
      memorySaver.changed,
    backup,
    localStateBackup: memorySaver.localStateBackup,
    sessionBackup,
    preferences,
    localState: memorySaver.localState,
  };
}
