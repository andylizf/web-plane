 ...
 * matter how correctly it was shown. A locked screen is the same shape with a
 * different number — only the lock window is composited. Read as a fact about
 * one window, either state accuses a perfectly visible window of sitting in the
 * Dock; that is the false positive this function exists to prevent (three of
 * them in four minutes on 2026-08-10, all inside one display-off interval, while
 * the window was on screen the whole time).
 *
 ...
 * can be staged in a unit test. `screenLocked` covers the case the count cannot:
 * with the display awake and the screen locked, the count is small but nonzero.
 *
