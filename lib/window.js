...
function findChrome(session, { unique = true } = {}) {
...
function screenWindows(pid) {
  const jxa = [
    ...
    'JSON.stringify(mine.map(function (w) { return { alpha: w.kCGWindowAlpha, left: w.kCGWindowBounds.X, top: w.kCGWindowBounds.Y, width: w.kCGWindowBounds.Width, height: w.kCGWindowBounds.Height } }))',
  ].join('; ');
  try {
    return JSON.parse(
...
function screenWindowAt(list, bounds) {
...
async function verifyShown(ws, chrome, windowIds) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    for (const windowId of windowIds) {
      ...
      if (!cur) {
        problems.push(`window ${windowId}: Chrome would not report its bounds`);
        continue;
      }
      if (cur.windowState !== 'normal') {
        problems.push(`window ${windowId}: still ${cur.windowState}`);
        continue;
      }
      if (isParked(cur)) {
        problems.push(`window ${windowId}: still parked offscreen at (${cur.left}, ${cur.top})`);
        continue;
      }
      if (!screen) continue; // bounds-only verification; reported by the caller
      const real = screenWindowAt(screen, cur);
      if (!real) {
        problems.push(
          `window ${windowId}: Chrome reports ${cur.width}x${cur.height} at (${cur.left}, ${cur.top}), ` +
            `but the window server has no window of that size there for pid ${chrome.pid}`
        );
      } else if (!(real.alpha > 0)) {
        problems.push(
