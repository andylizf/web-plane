...
import { stopLaneMonitor } from './lane-monitor.js';
import { forgetLane, listLaneStates } from './lane-state.js';
import { findChrome } from './procs.js';
import { livePageContextIds, splitContextMessage } from './browser-contexts.js';
import { clearBlockerOwners } from './ui.js';
...
/**
 * Chrome writes profile.exit_type=Crashed as soon as a profile opens. Its macOS
 * process does not reliably replace that value when automation asks it to
 * close, even when the process exits without SIGKILL. The next launch then
 * raises a persistent "Restore pages?" window for a shutdown the caller
 * explicitly requested.
 *
 * Only normalize profiles owned by web-plane, and only after an explicit close
 * has stopped Chrome. Genuine crashes still retain Chrome's recovery UI. The
 * small backup record makes the one-field mutation auditable without copying a
 * profile Preferences file full of account metadata into another directory.
 */
export function normalizeExplicitClose(chrome) {
  if (!chrome.session || !chrome.dir.startsWith(`${paths.profilesDir}/`)) {
    return { changed: false };
  }

  const preferences = join(chrome.dir, 'Default', 'Preferences');
  if (!existsSync(preferences)) return { changed: false };

  const original = readFileSync(preferences, 'utf8');
  const state = JSON.parse(original);
  if (state.profile?.exit_type !== 'Crashed') return { changed: false };

  const backupDir = join(paths.runtimeBackupsDir, 'profile-exit-state');
  mkdirSync(backupDir, { recursive: true });
  const token = `${Date.now()}-${randomUUID()}`;
  const backup = join(backupDir, `explicit-close-${token}.json`);
  writeFileSync(
    backup,
    JSON.stringify({
      session: chrome.session,
      preferences,
      previousExitType: state.profile.exit_type,
      nextExitType: 'Normal',
    }) + '\n',
    { mode: 0o600, flag: 'wx' }
  );
  ...
  const staged = `${preferences}.web-plane-${token}`;
  try {
    writeFileSync(staged, JSON.stringify(state), {
      mode: statSync(preferences).mode & 0o777,
      flag: 'wx',
    });
    renameSync(staged, preferences);
  } catch (error) {
...
export function hiddenFlagFor(chrome) {
...
// A shown window belongs to the user until they give it back: they may be in
// the middle of a handoff step for any agent sharing this Chrome. Intent (the
// hidden flag) decides, not the window server, so a window the user dragged
// behind another app still counts as theirs.
export async function sessionShown(chrome) {
  if (chrome.managed) return !isAlphaHidden(chrome);
  const { ws, wins } = await openWindowSession(chrome);
  ws.close();
  return !wins.every((win) => win.bounds?.windowState === 'minimized');
}

export const USER_ACTIVE_IDLE_MS = 120_000;

function hidIdleMs() {
  try {
    const out = execSync('ioreg -c IOHIDSystem -d 4', { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] });
...
function frontmostPid() {
  try {
    const jxa = 'ObjC.import("AppKit"); $.NSWorkspace.sharedWorkspace.frontmostApplication.processIdentifier';
    const pid = Number(execSync(`osascript -l JavaScript -e '${jxa}'`, { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).trim());
...
/** The user is working in this Chrome: it is the frontmost app and input is recent. */
export function userActiveDecision({ chromePid, frontPid, idleMs, thresholdMs = USER_ACTIVE_IDLE_MS }) {
  if (frontPid == null || idleMs == null) return { active: null, reason: 'user activity could not be measured' };
  if (frontPid !== chromePid) return { active: false, reason: 'another app is frontmost' };
  if (idleMs >= thresholdMs) return { active: false, reason: `no input for ${Math.round(idleMs / 1000)}s` };
  return { active: true, reason: `this browser is frontmost and the last input was ${Math.round(idleMs / 1000)}s ago` };
}

export function userActiveIn(chrome) {
  return userActiveDecision({ chromePid: chrome.pid, frontPid: frontmostPid(), idleMs: hidIdleMs() });
}

/** `web-plane -s=<name> wait-hidden [seconds]`: block until the user's window is hidden. */
export async function waitHidden(session, seconds = 600) {
  const chrome = findChrome(session);
  const deadline = Date.now() + seconds * 1000;
  while (await sessionShown(chrome)) {
    if (Date.now() >= deadline) {
      console.error(`web-plane: window still shown after ${seconds}s`);
      process.exitCode = 1;
      return;
    }
...
export function screenWindows(pid) {
  const jxa = [
    ...
    'var frontPid = null; try { frontPid = Number($.NSWorkspace.sharedWorkspace.frontmostApplication.processIdentifier) } catch (e) {}',
    'JSON.stringify({ systemOnScreen: onScreen.length, screenLocked: locked, frontmostPid: frontPid, windows: mine.map(function (w) { return { number: w.kCGWindowNumber, alpha: w.kCGWindowAlpha, left: w.kCGWindowBounds.X, top: w.kCGWindowBounds.Y, width: w.kCGWindowBounds.Width, height: w.kCGWindowBounds.Height, onScreen: Boolean(live[w.kCGWindowNumber]) } }) })',
  ].join('; ');
  try {
    return JSON.parse(
...
export function measuredFrontmostApp(pid, screen) {
  if (!screen || compositorBlindSpot(screen) || !Number.isInteger(screen.frontmostPid) || screen.frontmostPid <= 0) return null;
  return screen.frontmostPid === pid;
}
...
export function windowVisibilityUncertainty(windowId, cur, screen) {
  if (!cur || cur.windowState !== 'normal' || !screen || compositorBlindSpot(screen)) return null;
  const real = screenWindowAt(screen.windows, cur);
  if (real && real.alpha > 0 && !isParked(real) && !real.onScreen) {
    // Absence from the current compositor list does not distinguish another
    // Space from a minimized window. Status already reports this as unknown.
    return `window ${windowId}: visibility unknown; absent from the current on-screen list, ` +
      `with normal bounds and positive alpha. This does not establish that showing failed`;
  }
  return null;
}

// Did `show` actually put a visible window on screen? Returns the reasons it did
// not, plus `unchecked`: why the window-server half
...
async function verifyShown(ws, chrome, windowIds) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    for (const windowId of windowIds) {
      ...
      unchecked ??= windowVisibilityUncertainty(windowId, cur, screen);
    }
    if (!problems.length && !unchecked) return { problems, unchecked };
    await sleep(150);
  }
...
async function openWindowSession(chrome) {
  ...
  for (const p of pages) {
    const w = await cdpSend(ws, 'Browser.getWindowForTarget', { targetId: p.targetId });
    if (w.windowId && !wins.some((x) => x.windowId === w.windowId)) wins.push(w);
  }
  return { ws, wins, contextIds: livePageContextIds(pages) };
}

export async function windowControl(action, session = null, { whileActive = false } = {}) {
  const chrome = findChrome(session);
  ...
  const { ws, wins, contextIds } = await openWindowSession(chrome);
  ...
  if (doShow) {
    // A Workspace sign-in can create Profile 1 inside this session's
    // --user-data-dir. Both profiles then expose page targets through the same
    // CDP endpoint, and activating the Chrome process raises both windows. Stop
    // while the standing-hidden flag is still armed rather than hand the human
    // whichever profile Chrome happens to put in front.
    if (contextIds.length > 1) {
      ws.close();
      console.error(`\n${splitContextMessage(chrome.session ?? '(unnamed)', 'show', contextIds.length)}\n`);
      process.exitCode = 1;
      return;
    }
    ...
    if (!problems.length) {
      console.log(unchecked ? `Show requested (${count}); visibility unknown` : `Window shown (${count})`);
      if (unchecked) {
        ...
        console.error(
          `\nweb-plane: note — visibility could not be double-checked, either way:\n` +
            `  ${unchecked}.\n` +
            `  Chrome's window state and bounds were checked. This is not proof of visibility\n` +
            `  or invisibility. If the user can see the window, continue the handoff; do not\n` +
            `  restart their session based on this result.\n`
        );
      }
    } else {
    ...
    const activity = whileActive ? null : userActiveIn(chrome);
    if (activity?.active) {
      ws.close();
      console.error(JSON.stringify({
        ...
        error: {
          code: 'USER_ACTIVE',
          message: `the user is working in this window (${activity.reason}); it was not hidden`,
          next: 'Ask the user before hiding. Only after they agree: ' +
            `web-plane -s=${chrome.session ?? '<name>'} hide --while-active`,
        },
      }));
      process.exitCode = 4;
      return;
...
export async function getStatus(session = null) {
  ...
  try {
    ...
    ws.close();
    const screen = screenWindows(chrome.pid);
    windowState = measuredWindowState(wins, screen);
    frontmost = measuredFrontmostApp(chrome.pid, screen);
  } catch {}
  return {
    running: true,
    pid: chrome.pid,
...
export async function closeSession(session = null) {
  ...
  const laneCleanupFailures = [];
  const lanes = listLaneStates(chrome.session);
  for (const lane of lanes) {
    stopLaneMonitor(lane.lane);
    const daemon = await stopAgentBrowserDaemon(lane.lane);
    if (daemon.stopped) forgetLane(lane.lane);
    else laneCleanupFailures.push({ lane: lane.lane, reason: daemon.reason });
  }

  let normalized;
  let normalizeError = null;
  try {
    normalized = normalizeExplicitClose(chrome);
  } catch (error) {
  ...
  clearBlockerOwners(chrome.runId);

  if (normalizeError) {
    console.error(
      `Closed session '${chrome.session ?? '(unnamed)'}', but could not clear its false crash state: ` +
        normalizeError.message
    );
    return 1;
  }
  if (laneCleanupFailures.length) {
    console.error(
      `Closed session '${chrome.session ?? '(unnamed)'}', but lane driver cleanup failed: ` +
        laneCleanupFailures.map((failure) => `${failure.lane} (${failure.reason})`).join(', ')
    );
    return 1;
  }
  ...
  console.log(
    `Closed session '${chrome.session ?? '(unnamed)'}' (pid ${chrome.pid})` +
      ...
      (normalized.changed ? ' — cleared crash recovery for this requested close' : '') +
      (lanes.length ? ` — stopped ${lanes.length} lane driver(s)` : '')
  );
  return 0;
}
